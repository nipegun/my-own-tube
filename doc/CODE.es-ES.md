...
SQLite es el único almacén de metadatos. `backend/inc/db.php` centraliza PDO y las consultas preparadas; el modo WAL permite lecturas normales durante escrituras breves y todas las conexiones activan claves foráneas. `seed.php` crea el esquema inicial. Las migraciones idempotentes de `backend/inc/session.php` adaptan instalaciones antiguas conservando filas, contadores y contenido y asignando al administrador los datos privados que antes eran globales.

Los bytes de vídeo no se guardan en SQLite. Cada vídeo importado tiene el original, un JSON adyacente y normalmente una miniatura JPEG. SQLite conserva el catálogo, el estado de usuario, un `file_path` relativo seguro y `is_available`. La sincronización oculta las filas cuyos archivos desaparecen, conserva su estado de usuario y las reactiva cuando vuelve el contenido. `media.php` y `thumb.php` son pasarelas autenticadas; Apache deniega el acceso directo a la base y al directorio multimedia.

La autenticación utiliza sesiones PHP. Las contraseñas usan Argon2id y los cifrados antiguos se renuevan después de un acceso correcto. Cada sesión guarda el `session_version` de la cuenta en la base; los cambios y restablecimientos de contraseña lo incrementan para invalidar las sesiones antiguas. Las mutaciones, incluido el registro del historial de búsqueda, exigen POST y un token CSRF ligado a la sesión. La salida se escapa al renderizar y la limitación de accesos se identifica mediante un hash de la dirección cliente. El VirtualHost de Debian espera un proxy de confianza que envíe el protocolo PROXY de HAProxy para que Apache proporcione la dirección correcta.

La internacionalización utiliza catálogos JSON planos. El inglés sirve de reserva; la cuenta guarda el idioma del usuario y un ajuste global se aplica a cuentas nuevas. Los cuatro catálogos deben mantener las mismas claves.

El instalador también actualiza. Descarga por HTTPS, detiene Apache antes de copiar SQLite, verifica la copia, despliega desde un directorio de preparación, conserva los vídeos fuera del código, valida Apache y mantiene copias de reversión. Como cambia globalmente el MPM y el módulo PHP, rechaza primero los sitios habilitados ajenos y necesita una instancia de Apache dedicada. El usuario web solo puede escribir en base, vídeos y registros.
...
| Controlador de páginas | `backend/index.php` | Cabeceras, esquema, acceso, despacho y composición | Módulos `inc`, `pages` | Peticiones de páginas |
| Páginas | `backend/pages/*.php` | Consultar y renderizar una pantalla | Base, sesión, componentes, i18n | `backend/index.php` |
| Acciones | `backend/actions/*.php` | Validar POST y modificar estado | Base, sesión, ayudas, i18n, vídeo | Formularios y progreso |
...
| Dominio de vídeo | `backend/inc/videos.php` | Rutas seguras, ffprobe, metadatos, miniaturas e importación | Base, sesión, FFmpeg | Subida, sync, multimedia |
| Componentes | `frontend/components/components.php` | Tarjetas, filas, avatares y navegación de páginas con salida escapada | Ayudas, iconos, vídeo, i18n | Páginas |
...
| Iconos | `frontend/components/icons.php` | Funciones SVG en línea | Ninguno | Componentes y páginas |
| Ayudas | `backend/inc/helpers.php` | Escape, URL, paginación limitada en la base y formatos de presentación | i18n opcional | Renderizado y redirección |
| Pasarela multimedia | `backend/media.php` | GET/HEAD autenticado y un rango de bytes | Base, sesión, vídeo | Reproductor y descarga |
| Pasarela de miniaturas | `backend/thumb.php` | Entrega JPEG autenticada | Base, sesión, vídeo | Tarjetas y reproducción |
...
| Política Apache | `frontend/public/.htaccess` | Bloquear datos, medios directos y CLI | Apache | Web desplegada |
| Presentación | `frontend/public/styles.css` | Maquetación adaptable sin ocultar desbordes y temas claro/oscuro del sistema | Clases renderizadas | Todas las páginas |
...
| Ayuda de enlaces | `backend/tools/generate_symlink.py` | Enlazar un árbol externo sin solapamientos | Python 3.13, sistema de archivos | Operador antes del sync |
...
| `fQuery` | `backend/inc/db.php:45` | Ejecuta una consulta preparada sin revelar errores SQL al usuario |
| `fE` | `backend/inc/helpers.php:3` | Escapa texto para HTML |
| `fUrl` | `backend/inc/helpers.php:7` | Construye URL del controlador frontal |
| `fCsrfRequire` | `backend/inc/session.php:69` | Rechaza mutaciones sin el token de sesión correcto |
...
| `fAuthMigrateUserTable` | `backend/inc/session.php:278` | Reconstruye tablas privadas antiguas y copia sus filas con propietario |
| `fAuthLoginAllowed` | `backend/inc/session.php:520` | Comprueba y caduca el límite de intentos por dirección |
...
| `fAuthAdminUpdateRole` | `backend/inc/session.php:737` | Cambia el rol sin eliminar el último administrador |
...
| `fT` | `backend/inc/i18n.php:162` | Traduce con reserva inglesa y sustituye parámetros |
| `fVideoPathFromFilePath` | `backend/inc/videos.php:126` | Resuelve un archivo solo dentro del directorio de vídeos |
| `fVideoSaveJson` | `backend/inc/videos.php:180` | Guarda el JSON de forma atómica |
...
| `fChannelById` | `backend/inc/videos.php:644` | Guarda en caché los canales que utiliza el renderizado de vídeos |
...
Formulario → `POST actions/login.php` → `fCsrfRequire()` → `fAuthLoginAllowed()` → consulta preparada y `password_verify()` → contador de fallo o `fAuthStartSession()` → rotación del identificador y almacenamiento de `session_version` → 303 a un destino interno validado. Cada carga posterior de usuario compara esa versión con SQLite.
...
Formulario de ajustes o CLI de recuperación → comprobar credenciales cuando corresponda → cifrado Argon2id → incrementar `users.session_version`. El cambio web también rota la sesión actual y guarda la nueva versión; las demás sesiones, o todas tras la CLI, quedan inválidas en su siguiente petición.

### Creación de cuenta

Registro o formulario administrativo → guardia correspondiente → `fAuthCreateUser()` → validación de correo, contraseña y nombre → `password_hash()` → inserción. El registro público inicia la sesión; la creación administrativa vuelve a usuarios.

### Subida

Formulario → `upload-video.php` → sesión/CSRF → extensión y tamaño → destino sin colisión → `move_uploaded_file()` → `fVideoValidateFile()` → metadatos y JSON atómico → `fVideoImportJson()` y miniatura → reproducción. Un fallo elimina los archivos creados en ese intento.

### Sincronización

Formulario administrativo o CLI → `fVideosSyncFolder()` → comprobar `ffprobe` → capturar los identificadores existentes con archivo → enumerar extensiones → validar flujos → crear JSON ausente → importar → crear/renovar miniatura → registrar fallos por archivo → marcar transaccionalmente como no disponibles los identificadores iniciales ausentes de las importaciones correctas → devolver contadores. La reconciliación no elimina filas ni estado de usuario y no oculta subidas creadas después de la captura.

### Reproducción y progreso

Página de reproducción → `media.php` valida sesión, ID y ruta → interpreta un rango → libera el bloqueo de sesión → envía bloques de 1 MiB. Tras el evento `playing`, JavaScript publica el porcentaje con CSRF; una transacción crea historial e incrementa vistas solo en la primera inserción o actualiza el progreso existente.

### Migración de inicio

Entrada web/CLI → `fAuthEnsureSchema()` → tablas ausentes → idioma y versión de sesión del usuario → disponibilidad de vídeos → administrador → reconstrucción transaccional de tablas privadas antiguas → `user_id` para comentarios/listas → índices y marcas de migración. No se vacían filas ni contadores.
...
|---|---|---|
| `GET /index.php?page=login` | Formulario público de acceso | `backend/pages/login.php` |
| `GET /index.php?page=register` | Registro público condicional | `backend/pages/register.php` |
| `GET /index.php?page=home|trending|search` | Catálogo, tendencias o búsqueda de solo lectura | `backend/pages/{home,trending,search}.php` |
...
| `GET|HEAD /media.php?id=…[&download=1]` | Emisión/rangos autenticados | `backend/media.php` |
| `GET|HEAD /thumb.php?id=…` | Miniatura autenticada | `backend/thumb.php` |

### Mutaciones POST

Todas exigen CSRF; salvo acceso y registro condicional, también una sesión iniciada.
...
| `/actions/toggle-like.php`, `/actions/toggle-watchlater.php` | Colecciones privadas | Archivos homónimos |
| `/actions/save-progress.php` | Progreso JSON y primera visualización | Archivo homónimo |
...
| `/actions/create-playlist.php`, `/actions/update-playlist.php`, `/actions/delete-playlist.php`, `/actions/update-playlist-item.php` | CRUD y contenido de listas propias | Archivos homónimos |
| `/actions/upload-video.php` | Validar, guardar e importar subida | Archivo homónimo |
...
| Ruta/URL/comando | Handler | Archivo |
|---|---|---|
| `php backend/seed.php [--force] [--demo]` | Crear base; opcionalmente copiar y sustituir | `backend/seed.php` |
| `php backend/secure-admin-cli.php` | Rotar credenciales históricas y migrar | Archivo indicado |
| `php backend/reset-admin-password-cli.php [correo]` | Restablecer/promocionar, revocar sesiones y borrar límites | Archivo indicado |
...
| `python3 backend/tools/generate_symlink.py ORIGEN` | Enlazar árbol multimedia externo | Archivo indicado |
...
| `backend/inc/i18n.php` o una clave | Toda la interfaz y los cuatro JSON; una ausencia usa inglés |
| Extensiones/rutas de `backend/inc/videos.php` | Subida, sync, reproducción, miniaturas y registros existentes |
| Semántica de `videos.file_path`/`is_available` | Visibilidad, reproducción, descarga, miniatura, sync, progreso y estado conservado |
| Rangos de `media.php` | Búsqueda temporal, descargas y concurrencia de sesión |
...
| Rutas/permisos del instalador | Actualización, recuperación, Apache y límites de escritura |
| Reglas del enrutador local | Archivos del repositorio que puede exponer el servidor integrado |
| Protocolo PROXY | IP de límites/registros y aceptación de conexiones directas |

## 7. Puntos de extensión

### Añadir una página

1. Añade el nombre de ruta a la lista permitida en `backend/index.php`.
2. Crea `backend/pages/<nombre>.php` para consultas y lógica de acceso.
3. Crea `frontend/views/pages/<nombre>.php` para el HTML escapado e inclúyelo desde el controlador mediante `cFrontendPath`.
4. Añade la navegación en `frontend/components/chrome.php` y los textos en los cuatro catálogos `frontend/lang/*.json`.
5. Actualiza los mapas de rutas/módulos y comprueba la página con ambos comandos de pruebas.

### Añadir una mutación

1. Crea `backend/actions/<nombre>.php` y carga los módulos compartidos.
2. Exige POST, `fCsrfRequire()` y `fRequireLogin()` o `fRequireAdmin()`.
3. Valida tipo, longitud, propiedad y referencias; usa marcadores y transacción si hay varios pasos.
4. Incluye `fCsrfField()` en el formulario, un aviso traducido y una redirección 303.
5. Añade o amplía una regresión en `tests/run-tests.sh`; una página GET debe seguir siendo de solo lectura.

### Añadir un idioma

Incluye idioma y nombre en `backend/config.php`, crea un JSON con todas las claves inglesas y conserva los marcadores. Comprueba paridad y persistencia de cuenta/sitio y actualiza la documentación localizada.

### Añadir una colección por usuario

Usa una clave primaria que empiece por `user_id`, una clave foránea con la cascada prevista, filtra siempre por `fUserId()` y añade un índice para el orden. Si existía una tabla global, crea una migración transaccional que copie datos poblados.

### Añadir metadatos de vídeo o una CLI

Modifica juntos `fVideoCreateMetadata()` y `fVideoImportJson()` y mantén JSON atómico, rutas seguras y compatibilidad con campos ausentes. Una CLI debe rechazar SAPIs web, reutilizar módulos, devolver estado no nulo al fallar y quedar bloqueada por Apache.

## 8. Invariantes de datos y seguridad

- Ninguna ruta del catálogo se resuelve fuera de `cVideosPath`.
- Un vídeo importado reproducible necesita original y JSON; la miniatura exige además su JPEG.
...
- La extensión es solo un filtro inicial: `ffprobe` debe detectar códec y dimensiones positivas.
- Los JSON y bases nuevas se activan mediante renombrado atómico.
- Las consultas privadas incluyen `user_id` y las listas comprueban también su propietario.
...
- Las mutaciones iniciadas por usuarios usan POST con CSRF; los GET de resultados y sugerencias nunca registran historial de búsqueda. Los destinos de redirección se restringen al controlador.
- Los parámetros escalares rechazan valores con forma de array y los identificadores SQL dinámicos se limitan mediante listas explícitas.
- Las pasarelas cierran la sesión antes de enviar archivos largos.
- Las respuestas web ocultan SQL y rutas internas; el detalle se escribe en los registros.
